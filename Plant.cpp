...
Plant::Plant(int rowId, int utilityId, std::string plantName, std::string plantType, double installationYear,
...
// Getters
int Plant::getRowId() const {
    return rowId;
}

int Plant::getUtilityId() const {
    return utilityId;
}
...
double Plant::getInstallationYear() const {
    return installationYear;
}

double Plant::getCapacityMw() const {
    return capacityMw;
}

double Plant::getPeakDemandMw() const {
    return peakDemandMw;
}

double Plant::getPlantHoursConnectedWhileGenerating() const {
    return plantHoursConnectedWhileGenerating;
}

double Plant::getNetCapacityFavorableConditionsMw() const {
    return netCapacityFavorableConditionsMw;
}

double Plant::getNetCapacityAdverseConditionsMw() const {
    return netCapacityAdverseConditionsMw;
}

double Plant::getNetGenerationMwh() const {
    return netGenerationMwh;
}

// Setters
void Plant::setRowId(int rowId) {
    this->rowId = rowId;
}

void Plant::setUtilityId(int utilityId) {
    this->utilityId = utilityId;
}

void Plant::setPlantName(std::string plantName) {
    this->plantName = plantName;
}

void Plant::setPlantType(std::string plantType) {
    this->plantType = plantType;
}

void Plant::setInstallationYear(double installationYear) {
    this->installationYear = installationYear;
}

void Plant::setCapacityMw(double capacityMw) {
    this->capacityMw = capacityMw;
}

void Plant::setPeakDemandMw(double peakDemandMw) {
    this->peakDemandMw = peakDemandMw;
}

void Plant::setPlantHoursConnectedWhileGenerating(double plantHoursConnectedWhileGenerating) {
    this->plantHoursConnectedWhileGenerating = plantHoursConnectedWhileGenerating;
}

void Plant::setNetCapacityFavorableConditionsMw(double netCapacityFavorableConditionsMw) {
    this->netCapacityFavorableConditionsMw = netCapacityFavorableConditionsMw;
}

void Plant::setNetCapacityAdverseConditionsMw(double netCapacityAdverseConditionsMw) {
    this->netCapacityAdverseConditionsMw = netCapacityAdverseConditionsMw;
}

void Plant::setNetGenerationMwh(double netGenerationMwh) {
    this->netGenerationMwh = netGenerationMwh;
}
 ...
 */
std::ostream &operator<<(std::ostream &outs, const Plant &obj) {
    outs << std::fixed << std::right << std::setprecision(4) << std::setw(6) << obj.getRowId() << std::setw(12)
         << obj.getUtilityId() << std::left << "  " << std::setw(20) << obj.getPlantName() << std::setw(18)
         << obj.getPlantType() << std::right << std::setprecision(0) << std::setw(13) << obj.getInstallationYear()
         << std::setprecision(1) << std::setw(16) << obj.getCapacityMw() << std::setw(13) << obj.getPeakDemandMw()
         << std::setw(15) << obj.getPlantHoursConnectedWhileGenerating() << std::setw(15)
         << obj.getNetCapacityAdverseConditionsMw() << std::setw(15) << obj.getNetCapacityFavorableConditionsMw()
...
void capacityStats(std::vector<Plant> &objs) {
    ...
    std::cout << "The largest plant is " << maxPlant.getPlantName() << " with a capacity of "
              << maxPlant.getCapacityMw() << " MW" << std::endl << "The smallest plant is " << minPlant.getPlantName()
              << " with a capacity of " << minPlant.getCapacityMw() << " MW" << std::endl
              << "The total capacity of all plants in this dataset is " << sum << " MW" << std::endl
              << "The average capacity of all plants in this dataset is " << std::setprecision(4) << avg << " MW"
              << std::endl;
}
...
bool loadFromFile(std::string fn, std::vector<Plant> &objs) {
    ...
    while (fin && fin.peek() != EOF) {
        ...
        Plant plant(rowId, utilityId, plantName, plantType, installationYear, capacityMw, peakDemandMw,
...
void verifyAllFieldsSet(std::vector<Plant> &objs) {
    ...
    for (Plant &plant: objs) {
        ...
        if (plant.getUtilityId() == -1) {
            std::cout << "Bad value for utility ID at record " << i << std::endl;
        }
        std::string plantName = plant.getPlantName();
        if (plantName.compare("INVALID") == 0) {
            std::cout << "Bad value for plant name at record " << i << std::endl;
        }
        std::string plantType = plant.getPlantType();
        if (plantType.compare("INVALID") == 0) {
            std::cout << "Bad value for plant type at record " << i << std::endl;
        }
        double capacityMw = plant.getCapacityMw();
        if (capacityMw == -1) {
            std::cout << "Bad value for capacityMw at record " << i << std::endl;
        }
        double peakDemandMw = plant.getPeakDemandMw();
        if (peakDemandMw == -1) {
            std::cout << "Bad value for peakDemandMw at record " << i << std::endl;
        }
...
void PlantTest() {
    ...
    Plant testObject2 = Plant(5, 6, "TestPlant2", "Storage", 2000.0, 80.0, 70.0, 5000.0, 100.0, 80.0, 1000.0);
